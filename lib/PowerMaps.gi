...
BindGlobal("InduceAllFromCyclicSubgroup", function(GorUT, i)
  local cls, ncl, pm, cen, o, I, inds, len, store, chorb, null, 
        res, k, ch, l, a, j;
  cls := ConjugacyClasses(GorUT);
  ncl := Length(cls);
  pm := PowerMapsOfAllClasses(GorUT)[i];
  cen := [];
  for j in pm do
    cen[j] := Size(GorUT)/Size(cls[j]);
  od;
  o := Length(pm);
  I := Positions(pm, i)-1;
  if IsUTable(GorUT) then
    inds := RationalClassIndices(GorUT);
    len := Length(inds);
